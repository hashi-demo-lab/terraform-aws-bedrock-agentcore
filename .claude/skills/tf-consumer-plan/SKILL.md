...
6. Ask up to 5 clarification questions via `AskUserQuestion`. Must include:
   ...
   - **HCP Terraform project**: Which HCP Terraform project should the workspace be created in? (e.g., `sandbox`, `production`). This controls workspace isolation and team permissions. Default to `sandbox` for development.
   - **Environment and workspace**: Target workspace name, region, and credential pattern (dynamic credentials, assume_role)?
